...
func runWithIO(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	if showVersion {
		if backup || byDate || bySize || classify || configPath != "" || dryRun || excludeRaw != "" || ignoreFilePath != "" || includeRaw != "" || interactive || listCategories || overwrite || rename || renameAlias || skip || stats || undo || update || verbose || fs.NArg() > 0 {
	...
	if update {
		if backup || byDate || bySize || classify || configPath != "" || dryRun || excludeRaw != "" || ignoreFilePath != "" || includeRaw != "" || interactive || jsonOutput || listCategories || overwrite || rename || renameAlias || skip || stats || undo || verbose || fs.NArg() > 0 {
	...
	if classify {
		...
		printClassifications(stdout, results, loadedConfig)
		return 0
	}
	...
	if listCategories {
		...
		printCategoryList(stdout, definitions, loadedConfig)
		return 0
	}

	if undo {
		if backup || byDate || bySize || excludeRaw != "" || ignoreFilePath != "" || includeRaw != "" || interactive || overwrite || rename || renameAlias || skip {
...
func printCategoryList(out io.Writer, categories []CategoryDefinition, loadedConfig *LoadedConfig) {
...
func printClassifications(out io.Writer, results []classificationResult, loadedConfig *LoadedConfig) {
	for _, result := range results {
		if result.Destination != "" && result.Destination != result.Category {
			fmt.Fprintf(out, "%s: %s -> %s\n", result.Input, result.Category, result.Destination)
			continue
		}
		fmt.Fprintf(out, "%s: %s\n", result.Input, result.Category)
	}
	if loadedConfig != nil {
		fmt.Fprintf(out, "Loaded config from %s.\n", loadedConfig.Path)
	}
